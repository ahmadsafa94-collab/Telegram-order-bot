...
PAYMENTS_CHANNEL_ID_RAW = os.environ.get("PAYMENTS_CHANNEL_ID", "")
...
ADMIN_LABELS = [
    ...
    A_FIND_ORDER, A_FIND_CUSTOMER, A_STOCK, A_TICKETS, A_CUSTOMER_VIEW,
]
...
async def post_receipt_to_payments_channel(context: ContextTypes.DEFAULT_TYPE, order_id: int):
    """Posts the approved receipt image to the private Payments channel,
    captioned with the country/method, amount, customer name, and
    subscription. Isolated with its own error handling — a failure here
    must never affect the actual order confirmation."""
    ...
    if not photo_file_id:
        return  # nothing to post — e.g. paid via Stars, which has no receipt
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Text([BUY_LABEL, MY_SUBS_LABEL, BASKET_LABEL, ANNOUNCEMENTS_LABEL, JOIN_CHANNEL_LABEL, TICKET_LABEL, SUPPORT_LABEL]), main_menu_text))
