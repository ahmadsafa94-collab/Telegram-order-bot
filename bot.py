...
# require — CLIQ (Jordan) receipts often don't show one at all. For these,
# we instead check the receipt shows one of our known CLIQ aliases/
# recipient names, since that's the only way to confirm it was actually
# sent to us rather than someone else.
NO_REFERENCE_REQUIRED_METHODS = {"Jordan"}
JORDAN_RECIPIENT_IDENTIFIERS = [
    "WKS777", "WKS999", "WKS555",
    "WALEED SHAQFEH", "WALEED KHALID AHMAD SHAQFEH", "SHAQFEH",
...
CARD_FEE_ALLOWANCE = 5.0

# Our valid receiving crypto wallets — used both in the customer-facing
# payment instructions (CRYPTO_INSTRUCTIONS below) and to verify a
# submitted receipt actually paid to one of these addresses.
CRYPTO_WALLETS = {
    "USDT_BEP20": "0xefbab9265bb3a22492e4a100c27791da385eeb37",
    "USDT_TRC20": "TM8JhfHfaxNgBRs3benrTkWAz6Zprk9QmF",
    "BTC": "18rALgWKZZPMgGjVBCDBPgnnpdaj8rEQ2x",
}


async def get_live_btc_price_usd():
    """Fetches the current BTC/USD price from CoinGecko's free public API.
    Returns None on any failure (timeout, bad response, etc.) — callers
    MUST treat that as 'can't verify amount right now' and fall back to
    manual review, never guess a price or skip the check silently."""
    import aiohttp
    try:
        ...
        logger.exception("Failed to fetch live BTC price")
        return None
...
CRYPTO_INSTRUCTIONS = (
    ...
    f"*USDT (BEP20 Network)*\n`{CRYPTO_WALLETS['USDT_BEP20']}`\n\n"
    f"*USDT (TRC20 Network)*\n`{CRYPTO_WALLETS['USDT_TRC20']}`\n\n"
...
def queue_receipt_request(context: ContextTypes.DEFAULT_TYPE, order_id: int):
    """Registers that we're now expecting a receipt photo for order_id and
    makes it the IMMEDIATE target — i.e. the very next photo the customer
    sends is attributed to THIS order, not to some older order that's also
    still waiting on a receipt.

    Inserted at the front of the queue rather than appended: whichever
    order was just prompted for ("Order #38: please upload...") is what
    the customer is actually responding to, so it must win over anything
    older already sitting in the queue. Older still-outstanding orders
    stay queued behind it and get asked for in turn once this one's photo
    arrives — see receipt_photo(), which always drains from the front.

    Using a single shared helper for this (instead of three separate
    copies of the same few lines) is what previously let awaiting_receipt_for_order
    and the queue's actual head drift out of sync, causing receipts to be
    filed under the wrong order."""
    pending = context.user_data.setdefault("pending_receipt_orders", [])
    if order_id in pending:
        pending.remove(order_id)
    pending.insert(0, order_id)
...
def db_bulk_insert_serials(duration: str, codes: list) -> int:
    """Adds new serials from the reseller portal sync, skipping ones
    already in the pool (UNIQUE constraint on code). Returns how many
    were actually new."""
    conn = sqlite3.connect(DB_PATH)
    before = conn.execute("SELECT COUNT(*) FROM serials").fetchone()[0]
    conn.executemany(
        "INSERT OR IGNORE INTO serials (duration, code, status) VALUES (?, ?, 'available')",
        [(duration, code) for code in codes],
    )
    conn.commit()
    after = conn.execute("SELECT COUNT(*) FROM serials").fetchone()[0]
    conn.close()
    return after - before
...
def _country_instructions_view(order_id: int, country: str):
    ...
    if country == "Jordan":
        receipt_note = (
            "📸 *Send a screenshot of the receipt clearly showing the amount in JOD and "
            "the CLIQ ID/name you sent it to.*"
        )
    else:
        receipt_note = (
...
async def pay_card_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    order = db_get_order(order_id)
    total = order[4] if order else 0
    db_set_payment_method(order_id, "card")
    ...
    if action == "paid":
        ...
        if context.user_data.get("selected_country") == "India":
            ...
            text = (
                ...
                "the wallet address, and the transaction ID.*\n\n"
                "⚠️ Receipts that don't have this info won't be automatically checked "
                "and you will have to wait for manual check by the admins."
            )
        else:
            text = (
...
async def payer_name_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if verdict and verdict.get("verdict") == "looks_valid":
        ...
        amount_ok = (
            ...
            and (expected_amount - tolerance_low) <= extracted_amount <= (expected_amount + tolerance_high)
        )

        wallet_ok = True  # only actually checked for Cryptocurrency; stays True elsewhere
        if payment_method == "Cryptocurrency":
            wallet_ok = bool(verdict.get("wallet_ok"))
            if verdict.get("crypto_currency") == "BTC":
                # The generic amount_ok above compared a raw BTC amount
                # (e.g. 0.00085) against a USD total — meaningless. Live
                # price conversion replaces it entirely for BTC; a failed
                # price fetch means "can't verify" — falls to manual
                # review, never guesses or skips the check silently.
                btc_price = await get_live_btc_price_usd()
                if btc_price is None or extracted_amount is None:
                    amount_ok = False
                else:
                    expected_btc = expected_amount / btc_price
                    # Wider tolerance than fiat (5%) — BTC price moves
                    # between when the customer paid and when we check,
                    # and wallets often truncate/round the displayed amount.
                    btc_tolerance = expected_btc * 0.05
                    amount_ok = abs(extracted_amount - expected_btc) <= btc_tolerance

        if payment_method in NO_REFERENCE_REQUIRED_METHODS:
            # Jordan/CLIQ: no transaction reference is realistic to
            # require, so we rely on the AI having confirmed the receipt
            # shows one of our known CLIQ aliases/recipient names instead.
            # No duplicate check here — the recipient is legitimately the
            # same on every genuine receipt, so it can't be used as a
            # fraud signal the way a unique transaction reference can.
            reference_ok = bool(verdict.get("recipient_ok"))
            duplicate_ref = None
        else:
            # Every item needs its own distinct reference.
...
async def analyze_receipt_with_ai(context: ContextTypes.DEFAULT_TYPE, items: list,
    ...
    try:
        ...
        jordan_note = (
            " This is a Jordan/CLIQ payment. CLIQ receipts often do NOT show any transaction "
            "reference number at all — that is normal for this payment method, do NOT require "
            "one. Instead, check whether the receipt shows the amount in JOD AND the recipient "
            f"matches one of these known names/aliases: {', '.join(JORDAN_RECIPIENT_IDENTIFIERS)}. "
            "Matching rules: case-insensitive; a SURNAME appearing anywhere in the recipient's "
            "full name is enough on its own (e.g. if the receipt shows \"MOHAMMAD ISMAIL "
            "ABEDELQADER SHAMALTY\" and \"SHAMALTY\" is in the list, that's a match — the extra "
            "middle names don't matter); minor spelling variants are fine too (e.g. \"SHAMALTY\" "
        ...
        card_note = (
            ...
            f"payment processor's transaction fee — do NOT flag the amount as a mismatch just "
            f"because it's a few dollars above {expected_amount:.2f}; that's expected. Only flag "
        ...
        crypto_note = (
            ...
            f"  USDT (TRC20): {CRYPTO_WALLETS['USDT_TRC20']}\n"
            f"  Bitcoin (BTC): {CRYPTO_WALLETS['BTC']}\n"
            "Check whether the receipt shows the payment going to ONE of these exact addresses "
            "(minor case differences in the 0x... hex address are fine, but the characters "
            "themselves must match — don't guess or approximate a close-but-different address as "
            "a match). Set \"wallet_ok\" to true only if you can clearly see one of these exact "
            "addresses as the destination; false if the address shown doesn't match any of them, "
            "is missing, or is unreadable. Also set \"crypto_currency\" to \"USDT\" or \"BTC\" "
            "based on which was sent. IMPORTANT: if crypto_currency is \"BTC\", do NOT judge "
            "whether the amount matches the required total yourself — you don't have a live "
            "BTC/USD exchange rate, so that conversion will be done separately by our own code. "
            "Just report the raw BTC amount exactly as shown (e.g. \"0.00085\") in total_amount, "
            "and do not mark the verdict \"looks_off\" purely because of the BTC amount — base "
            "the verdict only on wallet_ok and whether a transaction reference is visible for "
            "BTC receipts specifically. For USDT, judge the amount normally against the required "
            "total as usual, since 1 USDT ≈ 1 USD."
            if payment_method == "Cryptocurrency" else ""
        )
        multi_note = (
        ...
        reference_requirement = (
            "This payment method does not require a transaction reference — see the note above "
            "about what to check instead."
            if payment_method in NO_REFERENCE_REQUIRED_METHODS else
            "Every receipt MUST show at least one transaction reference — a UTR, transaction ID, "
            "reference number, or confirmation code. If a receipt has no such reference visible "
            "anywhere, that alone makes it invalid, regardless of the amount."
        )
        ...
        content.append({"type": "text", "text": (
            ...
            '"recipient_ok": true or false, '
            '"wallet_ok": true or false, '
            '"crypto_currency": "USDT" or "BTC" or null, '
            '"reason": "<your final conclusion in under 15 words>"}\n\n'
            ...
            "currency, and not explained by a normal card processing fee — and NOT applicable to "
            "BTC amounts, see above), any image doesn't look like a real payment receipt, the "
            "exact same receipt image appears to be submitted more than once to inflate the "
            "total, or the reference/recipient/wallet requirement described above isn't met. "
            ...
            "for card, normal fees; not applicable to BTC) and the reference/recipient/wallet "
        ...
        try:
            ...
            logger.error("AI receipt analysis returned non-JSON — raw response: %s", raw[:500])
            if ADMIN_CHAT_ID:
                try:
                    await context.bot.send_message(
                        chat_id=ADMIN_CHAT_ID,
                        text=(
                            "⚠️ AI receipt check failed to parse — falling back to manual review "
        ...
        if ADMIN_CHAT_ID:
            try:
                await context.bot.send_message(
                    chat_id=ADMIN_CHAT_ID,
                    text=f"⚠️ AI receipt check failed ({type(e).__name__}: {e}) — falling back to manual review.",
                )
            except Exception:
                pass
        return None
...
def _ai_verdict_label(verdict: dict, payment_method: str = None) -> str:
    ...
    if verdict.get("crypto_currency"):
...
async def sync_imd_reseller_serials(username: str, password: str, status_cb,
                                     bot=None, admin_id=None) -> dict:
    """Logs into the iMD reseller portal (sg.imedicaldoctor.net —
    separate site/account from the imdweb.org customer portal used for
    the catalog and registration automation), exports the 'Unused' 6
    Month and 1 Year serial lists as CSV, and returns the parsed codes.
    Does NOT write to the database — the caller decides what to insert,
    so this stays a pure fetch step that's easy to debug on its own if
    the site's markup doesn't match what we expect."""
    ...
    async with async_playwright() as p:
        ...
        # ── Login ─────────────────────────────────────────────────────
        await status_cb("🔑 Logging in to the iMD reseller portal...")
        await page.goto("https://imedicaldoctor.net/reseller.php?a=my_resellers",
                         wait_until="load", timeout=60000)
        await page.wait_for_timeout(2000)

        page_text = await page.content()
        if "Cloudflare" in page_text and ("Verify you are human" in page_text or "security verification" in page_text.lower()):
            # Explicitly NOT attempting to solve or click through this —
            # that would mean building a bot-detection bypass, which is
            # off the table regardless of legitimate intent. Report it
            # clearly instead so this stays a known, deliberate limit
            # rather than a mysterious failure.
            if bot and admin_id:
                try:
                    sc = "/tmp/imd_reseller_cloudflare.png"
                    await page.screenshot(path=sc)
                    await bot.send_photo(
                        chat_id=admin_id, photo=open(sc, "rb"),
                        caption="🛑 Hit a Cloudflare bot-verification challenge on this URL too. "
                                "Not something this can or should try to get past automatically.",
                    )
                except Exception:
                    pass
            await browser.close()
            raise ValueError("Blocked by Cloudflare's bot verification — this URL is also protected.")
        ...
        clicked = False
        for sel in ['button:has-text("Sign In")', 'button[type="submit"]', 'input[type="submit"]']:
            if await page.locator(sel).count():
                await page.locator(sel).first.click()
                clicked = True
                break
        ...
        if await page.locator('input[type="password"]').count():
            # Still on a login form — didn't get past it.
            if bot and admin_id:
                try:
                    sc = "/tmp/imd_reseller_login_fail.png"
                    await page.screenshot(path=sc)
                    await bot.send_photo(chat_id=admin_id, photo=open(sc, "rb"),
                                         caption=f"Reseller login failed. URL: {page.url}")
                except Exception:
                    pass
            await browser.close()
            raise ValueError("Reseller portal login failed — check the username and password.")

        await status_cb("📄 Opening My Serials...")
        for tab_sel in ['a:has-text("My Serials")', 'button:has-text("My Serials")']:
            if await page.locator(tab_sel).count():
                await page.locator(tab_sel).first.click()
                break
        await page.wait_for_timeout(2000)

        # ── One export per duration filter ──────────────────────────
        filter_map = {"6m": "Unused · 6 Month", "1y": "Unused · 1 Year"}
        for duration, filter_label in filter_map.items():
            await status_cb(f"📥 Exporting {filter_label}...")
            filter_btn = page.locator(f'button:has-text("{filter_label}")')
            if await filter_btn.count():
                await filter_btn.first.click()
                await page.wait_for_timeout(1500)
            else:
                # Don't fail silently — grab every button's actual text so
                # we can see exactly what the site really calls this
                # filter instead of guessing again.
                all_buttons = await page.eval_on_selector_all(
                    "button", "els => els.map(e => e.innerText.trim()).filter(t => t)"
                )
                logger.warning("Quick filter button '%s' not found. Buttons on page: %s",
                                filter_label, all_buttons)
                if bot and admin_id:
                    try:
                        sc = f"/tmp/imd_reseller_filter_fail_{duration}.png"
                        await page.screenshot(path=sc, full_page=True)
                        await bot.send_photo(
                            chat_id=admin_id, photo=open(sc, "rb"),
                            caption=(
                                f"⚠️ Couldn't find the '{filter_label}' filter button.\n\n"
                                f"Buttons actually on the page:\n" +
                                "\n".join(f"• {b}" for b in all_buttons[:20])
                            ),
                        )
                    except Exception:
                        pass
            ...
            export_btn = page.locator('button:has-text("Export CSV"), a:has-text("Export CSV")')
            if not await export_btn.count():
                logger.warning("Export CSV button not found for %s.", filter_label)
                if bot and admin_id:
                    try:
                        sc = f"/tmp/imd_reseller_noexport_{duration}.png"
                        await page.screenshot(path=sc, full_page=True)
                        await bot.send_photo(chat_id=admin_id, photo=open(sc, "rb"),
                                             caption=f"⚠️ Export CSV button not found after filtering {filter_label}.")
                    except Exception:
                        pass
                continue
            ...
            try:
                ...
                logger.exception("CSV export/download failed for %s", filter_label)
                if bot and admin_id:
                    try:
                        sc = f"/tmp/imd_reseller_export_fail_{duration}.png"
                        await page.screenshot(path=sc)
                        await bot.send_photo(chat_id=admin_id, photo=open(sc, "rb"),
                                             caption=f"CSV export failed for {filter_label}.")
                    except Exception:
                        pass
                continue
            ...
            if rows:
                ...
                for i, h in enumerate(header):
                    if "serial" in h:
                        ...
                        break
                for row in rows[1:]:
                    if row and len(row) > serial_col and row[serial_col].strip():
                        results[duration].append(row[serial_col].strip())

            if not results[duration]:
                logger.warning("CSV for %s parsed to 0 serials. Raw content: %s", filter_label, csv_text[:500])
                if bot and admin_id:
                    try:
                        await bot.send_message(
                            chat_id=admin_id,
                            text=(
                                f"⚠️ {filter_label} CSV downloaded but parsed to 0 serials.\n\n"
                                f"Raw content (first 500 chars):\n```\n{csv_text[:500]}\n```"
                            ),
                            parse_mode=ParseMode.MARKDOWN,
                        )
                    except Exception:
...
async def sync_serials_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only /syncserials: logs into the iMD reseller portal, pulls
    the unused 6-month and 1-year serial lists, and adds any new ones to
    the bot's own pool (existing ones are skipped automatically)."""
    if update.effective_user.id != ADMIN_CHAT_ID:
        return
    clear_admin_flow_state(context.user_data)
    context.user_data["awaiting_sync_serials_username"] = True
    await update.message.reply_text(
        "🔄 Sync Serials from Reseller Portal\n\n"
        "This logs into sg.imedicaldoctor.net and pulls your unused 6-month "
        "and 1-year serials into the bot's pool. Existing serials are skipped "
        "automatically — safe to run anytime.\n\n"
        "Send your reseller portal username:"
    )


async def sync_serials_username_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.pop("awaiting_sync_serials_username", None)
    context.user_data["sync_serials_username"] = update.message.text.strip()
    context.user_data["awaiting_sync_serials_password"] = True
    await update.message.reply_text(
        "Send your reseller portal password:\n_(not stored — used once for this sync)_",
        parse_mode=ParseMode.MARKDOWN,
    )


async def sync_serials_password_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.pop("awaiting_sync_serials_password", None)
    username = context.user_data.pop("sync_serials_username", "")
    ...
    status_msg = await context.bot.send_message(chat_id=ADMIN_CHAT_ID, text="🔄 Starting sync...")
    ...
    try:
        results = await sync_imd_reseller_serials(
            username, password, update_status, bot=context.bot, admin_id=ADMIN_CHAT_ID,
        )
        added_6m = db_bulk_insert_serials("6m", results["6m"])
        added_1y = db_bulk_insert_serials("1y", results["1y"])
        await update_status(
            f"✅ Sync complete.\n\n"
            f"6 Months: {len(results['6m'])} found on the portal, {added_6m} new added\n"
            f"1 Year: {len(results['1y'])} found on the portal, {added_1y} new added\n\n"
            f"Pool now has {db_count_serials('6m')} available 6-month and "
            f"{db_count_serials('1y')} available 1-year serials."
        )
    except ValueError as e:
        await update_status(f"❌ {e}")
    except Exception as e:
        logger.exception("Serial sync failed")
        await update_status(f"❌ Sync failed: {e}")
...
async def text_state_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id == ADMIN_CHAT_ID:
        ...
        if context.user_data.get("awaiting_imd_diag_password"):
            ...
            return
        if context.user_data.get("awaiting_sync_serials_username"):
            await sync_serials_username_reply(update, context)
            return
        if context.user_data.get("awaiting_sync_serials_password"):
            await sync_serials_password_reply(update, context)
            return
...
async def run_imd_registration(context: ContextTypes.DEFAULT_TYPE, order_id: int):
    ...
    if status == "serial_error":
        ...
        # Read-only check (db_count_serials doesn't reserve anything) — if
        # we called db_pop_serial here just to preview the next serial, it
        # would mark that serial 'pending' and it would then sit unused
        # forever, since the recursive retry below pops its own serial
        # independently. This just checks whether one exists.
        if db_count_serials(duration) > 0:
            await context.bot.send_message(
                chat_id=ADMIN_CHAT_ID,
                text=(
                    f"🔄 Serial `{serial_code}` failed for order #{order_id} — "
                    f"marked as not working, trying the next available serial..."
                ),
            )
...
async def web_app_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "send_receipt":
        ...
        if not oid:
            ...
            return
        queue_receipt_request(context, oid)
        await update.message.reply_text(
    ...
    if pay_lower == "stars":
        ...
        amount_stars = max(1, round(total * STAR_RATE))
        try:
            await update.message.reply_invoice(
                ...
                provider_token = "",  # empty string is required for Telegram Stars (XTR)
                currency   = "XTR",
                prices     = [LabeledPrice(label="Total", amount=amount_stars)],
            )
        except Exception as e:
            ...
            if ADMIN_CHAT_ID:
                try:
                    await context.bot.send_message(
                        chat_id=ADMIN_CHAT_ID,
                        text=f"⚠️ Stars invoice failed for order #{order_id}: {type(e).__name__}: {e}",
                    )
                except Exception:
                    pass
            await update.message.reply_text(
