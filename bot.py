...
# Green "SHOP" banner shown above the persistent keyboard on /start — a
# real Telegram button can't be colored, so this image does the visual
# work and the plain 🏪 Shop button underneath does the actual tapping.
SHOP_BANNER_PATH = os.path.join(os.path.dirname(__file__), "assets", "shop_banner.png")
...
async def send_shop_banner(bot, chat_id: int):
    """Sends the green SHOP banner right before the main menu keyboard —
    resent every time that keyboard is shown, since the actual Shop
    button itself has no way to be colored (see main_menu_keyboard).
    Silently does nothing if the asset is missing, so a missing file
    never breaks the actual menu underneath it."""
    if not os.path.exists(SHOP_BANNER_PATH):
        return
    try:
        with open(SHOP_BANNER_PATH, "rb") as f:
            await bot.send_photo(chat_id=chat_id, photo=f)
    except Exception:
        logger.exception("Failed to send shop banner")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.exists(SHOP_BANNER_PATH):
        try:
            with open(SHOP_BANNER_PATH, "rb") as f:
...
async def show_announcements(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not keyboard:
        await send_shop_banner(context.bot, user_id)
        await update.message.reply_text(
...
async def analyze_receipt_with_ai(context: ContextTypes.DEFAULT_TYPE, items: list,
    ...
    try:
        ...
        content.append({"type": "text", "text": (
            ...
            f"{india_note}{jordan_note}{card_note} "
            ...
            "for every other payment method just set it to false, it's ignored.\n\n"
            "Use \"looks_off\" if the total amount is meaningfully off (more than 1 unit of "
            "currency, and not explained by a normal card processing fee), any image doesn't look "
            "like a real payment receipt, the exact same receipt image appears to be submitted "
            "more than once to inflate the total, or the reference/recipient requirement described "
            "above isn't met. "
            "Use \"unclear\" if an image is blurry/cropped/ambiguous rather than confidently wrong. "
            "Use \"looks_valid\" if the total amount matches (allowing for normal rounding and, for "
            "card, normal fees) and the reference/recipient requirement described above is met."
        )})
...
async def admin_menu_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == A_VIEW_SERIALS:
        ...
        await send_shop_banner(context.bot, update.effective_user.id)
        await update.message.reply_text(
